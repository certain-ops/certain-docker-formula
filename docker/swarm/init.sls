...
# Swarm: {{ salt['cmd.run']("docker info 2>/dev/null | grep Swarm | awk '{print $2}'", python_shell=True) }}
...
{%- if salt['cmd.run']("docker info 2>/dev/null | grep Swarm | awk '{print $2}'", python_shell=True) != 'active' %}
include:
